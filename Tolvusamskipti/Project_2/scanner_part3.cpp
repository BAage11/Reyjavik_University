...
int KnockOnPorts() 
{
    ...
    // Loop through the five ports, and send the 'secret phrase' message
    int count = 1;      // for print output
    for(int index = 0; index < 5; index++) 
    {
        std::cout << "Sending 'secret phrase' message to port number " << ports[index] << "...";
        ...
        // Send message to socket with 'secret phrase' message
    ...
    // Initalizing variables to retrieve the secret message from last port
    int last_sock;
    char buffer[1000];
    memset(buffer, 0, sizeof(buffer)); 
    socklen_t length;
    ...
    if(msg_recv < 0) 
    {
        std::cout << "\nMessage retrieved from port number " << ports[4] << "..." << std::endl;
        std::cout << "The message is: 'You have knocked, you may enter.'" << std::endl;      // Taken from Canvas
        printf("-------------------------\n");
    ...
    else
    {
        ...
        std::cout << "\nMessage retrieved from port number " << ports[4] << " (last port)..." << std::endl;
        std::cout << "The message is: '" << lastport_content << "'" << std::endl;
        printf("-------------------------\n");
