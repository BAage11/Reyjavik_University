...
#include <stdio.h>              // Standard library function for file_input and output.
...
// Open a socket for communication (via a connection) between server and client,
// sending command from client, and printing out the output given from server side.
int main() {
    ...
    char buffer[5000];                      // Initialize buffer (block of memory)
    ...
    do {
        ...
        if (userCommand == "q" || userCommand == "Q") {
            ...
            send(sock_fd, userCommand.c_str(), userCommand.size(), 0);

            // Wait for response from server, and then output message from server
            memset(buffer, 0, sizeof(buffer));
            int messageRec = recv(sock_fd, buffer, sizeof(buffer), 0);
            cout << "Output:\n" << string(buffer, messageRec) << endl;
        }      
