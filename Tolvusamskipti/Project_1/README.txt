...
Compile code:
	...
	4. Code should now have compiled and ready to be used.
...
Run submitted code:
	...
	5. Now that the open ports have been found, the second part of the program can be run (scanner2) for solving the puzzle ports.
	   Run the following command line in terminal:

		sudo ./scanner2			[press enter]

	As shown, the command is done with sudo-privilege (root permission). That is, to be able to run the command as 'any user', with the default generally being the root.
	...
	7. The sequence of knocks and what ports are being contacted are displayed,
	   as well as the final message by the last port in the sequence.


	
	Nota Bene: 
		Was not able to retrieve the 'secret phrase' in part 2 of the program (scanner2), 
	    	and therefore was not able to retrieve the final message in part 3.
