...
def number_already_chosen(number):
  number_exists = False
  for value in board:
...
def check_draw():
  number_exists = False
  for value_board in board:
    for value_line in value_board:
      try:
        if int(value_line):
          number_exists = True
      except ValueError:
...
def player_turn(player):
  while True:
    ...
    if number_already_chosen(number) == True:
...
def main():
  ...
  while True:
    ...
    if draw == True:
      ...
      break
    print_out_board()
    player = player_turn(player_turn)
